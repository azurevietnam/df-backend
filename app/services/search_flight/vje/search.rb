class SearchFlight::Vje::Search
  def initialize(search_params)
    uri            = 'https://book.vietjetair.com'
    @search_params = search_params
    http_service   = Http::HttpFactory.new_http
    option_builder = SearchFlight::Vje::OptionBuilder.new(uri, search_params)
  ...
  private
    ...
    def parse_content(html_content, search_params)
      parser           = SearchFlight::Vje::Parse.new(html_content, search_params)
      flight_container = parser.parse()
      flight_container
    end
    def calculate_fare(flight_container)
      fare             = SearchFlight::Vje::Fare.new(flight_container)
