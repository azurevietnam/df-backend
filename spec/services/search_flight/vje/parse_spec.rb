...
RSpec.describe SearchFlight::Vje::Parse do
  describe 'parse flight info from html content' do
    ...
    context 'parse successfully for one way html content' do
      ...
      parser = SearchFlight::Vje::Parse.new(content, params)

      it 'return correct number of flights' do
        flight_container = parser.parse
        expect(flight_container.depart_flights.count).to eq(11)
      end
    end
    ...
    context 'parse successfully for round trip html content' do
      ...
      params        = SearchFlight::SearchParams
                      .new(@ori_code, @des_code, round_type, @depart_date,
                      @return_date, @adult.to_s, @child.to_s, @infant.to_s)
      content = File.open(Rails.root.join('spec', 'services', 'search_flight', 'vje', 'use_cases', 'round_trip.html')) { |f| f.read}
      parser = SearchFlight::Vje::Parse.new(content, params)

      it 'return correct number of flights' do
        flight_container = parser.parse
        expect(flight_container.depart_flights.count).to eq(27)
        expect(flight_container.return_flights.count).to eq(28)
      end
    end
  end
end
